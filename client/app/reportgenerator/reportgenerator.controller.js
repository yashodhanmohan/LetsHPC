...
\\maketitle\n`;
    ...
    generate_report() {
        ...
        var tBCDSI = `\\subsection{Brief and clear description about the Serial implementation}\n${this.iBCDSI}\n`;
        var tBCSPI = `\\subsection{Brief and clear description about the implementation of the approach (Parallelization Strategy, Mapping of computation to threads)}\n${this.iBCSPI}\n`;
        
        var tCoSeCo = `\\subsection{Complexity of serial code}\n${this.iCoSeCo}\n`;
        var tCoPaCo = `\\subsection{Complexity of parallel code (split as needed into work, step, etc.) }\n${this.iCoPaCo}\n`;
        var tCoPa = `\\subsection{Cost of Parallel Algorithm}\n${this.iCoPa}\n`;
        var tThSpu = `\\subsection{Theoretical Speedup (using asymptotic analysis, etc.)}\n${this.iThSpu}\n`;
        var tEstSerFra = `\\subsection{Estimated Serial Fraction }\n${this.iEstSerFra}\n`;
        var tTub = `\\subsection{Tight upper bound based on Amdahl\'s Law}\n${this.iTub}\n`;
        var tNuMA = `\\subsection{Number of memory accesses}\n${this.iNuMA}\n`;
        var tNoCom = `\\subsection{Number of computations}\n${this.iNoCom}\n\n`;
        
        var tTCRAProc = `\\subsection{Time Curve related analysis (as no. of processor increases)}\n${this.iTCRAProc}\n`;
        var tTCRAProSize = `\\subsection{Time Curve related analysis (as problem size increases, also for serial)}\n${this.iTCRAProSize}\n`;
        var tSCRA = `\\subsection{Speedup Curve related analysis (as problem size and no. of processors increase)}\n${this.iSCRA}\n`;
        var tECRA = `\\subsection{Efficiency Curve related analysis}\n${this.iECRA}\n`;
        var tKFRA = `\\subsection{Karp-Flatt metric related analysis}\n${this.iKFRA}\n\n`;
        
        var tMSPO = `\\subsection{Major serial and parallel overheads}\n${this.iMSPO}\n`;
        var tMWRA = `\\subsection{Memory wall related analysis}\n${this.iMWRA}\n`;
        var tCCRA = `\\subsection{Cache coherence related analysis}\n${this.iCCRA}\n`;
        var tFSRA = `\\subsection{False sharing related analysis}\n${this.iFSRA}\n`;
        var tSraRA = `\\subsection{Scheduling related analysis}\n${this.iSraRA}\n`;
        var tLBRA = `\\subsection{Load balance analysis}\n${this.iLBRA}\n`;
        var tSyncRA = `\\subsection{Synchronisation related analysis}\n${this.iSyncRA}\n`;
        var tGRA = `\\subsection{Granularity related analysis}\n${this.iGRA}\n`;
        var tScaRA = `\\subsection{Scalability related analysis}\n${this.iScaRA}\n\n`;
    
        var tAOC = `\\subsection{Analysis of any other concepts/factors you think were important in your problem-approach combination}\n${this.iAOC}\n`;
        var tCBMB = `\\subsection{Further details (Code balance , machine balance analysis, how much of peak performance achieved in terms of \\%)}\n${this.iCBMB}\n`;
        var tADDADA = `\\subsection{Advantages/Disadvantages of your approach}\n${this.iADDADA}\n`;
        var tDFIA = `\\subsection{Difficulties faced while implementing this approach}\n${this.iDFIA}\n`;
        var tAAC = `\\subsection{Additional Comments}\n${this.iAAC}\n\n`;
    ...
    // File related function =============================================
        ...
        check_for_errors() {
            ...
            if(this.perfData && this.numberData) {
                var all_n_time =  _.map(_.uniq(_.map(_.filter(this.uploadedTimeData, selectedNumber => {
                        return selectedNumber;
                    }), 'n')), _.toString);
                var all_n_perf =  _.map(_.uniq(_.map(_.filter(this.uploadedPerfData, selectedNumber => {
                        return selectedNumber;
                    }), 'n')), _.toString);;
                var all_p_time =  _.map(_.uniq(_.map(_.filter(this.uploadedTimeData, selectedNumber => {
                        return selectedNumber;
                    }), 'p')), _.toString);;
                var all_p_perf =  _.map(_.uniq(_.map(_.filter(this.uploadedPerfData, selectedNumber => {
                        return selectedNumber;
                    }), 'p')), _.toString);;
                ...
                if(all_n_time.length!=all_n_perf.length) {
                    ...
                    for(var i=0; i<all_n_time.length; i++) {
                ...
                if(all_p_time.length!=all_p_perf.length) {
                    ...
                    for(var i=0; i<all_p_time.length; i++) {
        ...
        getProblemData(numberFile, perfFile) {
            ...
            var numberFetch = numberFile;
            var perfFetch = perfFile;
        ...
        google_chart_object(all_p, data, plotFor, parameter, statistic) {
            ...
            var allData = {};
            var serial_data = {};
            var all_n = _.map(_.uniq(_.map(_.filter(data, selectedNumber => {
                        return selectedNumber;
                    }), 'n')), _.toString);
            ...
            for (var j=0; j<all_p.length; j++)                       //  addColumn(type, label, id) label and id are optional, all values are initially set to null
        ...
        file_to_object_perf(file_text) {
            ...
            for (var i in lines) {
                ...
                if(!isNaN(line[incrementer])) {
                    var number = {}
                    for (var k =incrementer; k<line.length; k++) {
